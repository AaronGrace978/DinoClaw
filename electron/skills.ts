...
const BUILT_IN_SKILL_PACKS: Skill[] = [
  ...
  createSkillPack({
    id: 'builtin-desktop-operator',
    name: 'Desktop Operator',
    category: 'automation',
    description: 'Operates real desktop apps by inspecting windows, focusing the right target, capturing screenshots, moving the cursor, clicking, and typing into the focused control. Use for OS-level copilot tasks outside the DinoClaw browser.',
    version: '1.0.0',
    author: 'DinoClaw Core',
    builtin: true,
    enabled: true,
    triggers: ['desktop', 'window', 'screen', 'mouse', 'cursor', 'focus app', 'focus window', 'type here', 'open file', 'reveal file'],
    tools: [
      'desktop_open_app',
    ...
    instructions: [
      'Desktop work is stateful. Inspect the environment before acting.',
      'If the target app is not open, launch it and wait for its window before trying to focus it.',
      'Prefer focusing the correct window before moving or typing.',
      'Use screenshots or window lists to ground your next action instead of guessing.',
      'Treat the focused app as the source of truth and verify progress step by step.',
    ].join('\n'),
    workflow: [
      'Launch the target app if needed, then wait for its window to appear.',
      'List windows or capture a screenshot to understand the current desktop.',
      'Focus the target app window by title or process name.',
      'Move the cursor with intent, click only after confirming target context, then type into the focused control if needed. Use keys mode for visible real-time typing.',
      'Use desktop_press_key, desktop_hotkey, and desktop_scroll for browser/app navigation like Enter, Tab, Ctrl+L, Ctrl+T, PageDown, and wheel scrolling.',
      'After each major action, inspect again before continuing.',
    ],
    recovery: [
      'If the wrong app is active, re-focus the intended window before typing.',
      'If coordinates are uncertain, inspect first with screenshot or cursor position rather than clicking blind.',
      'If the desktop changed unexpectedly, rebuild your plan from the new visible state.',
    ],
    outputStyle: [
      'Describe the real desktop target clearly.',
      'Be explicit about which app/window is being focused and why.',
    ],
    examples: [
      'Focus Visual Studio Code, open a file, and type into the active editor.',
      'Inspect the desktop, move to a target area, and click after confirming the right window is active.',
    ],
  }),
  createSkillPack({
